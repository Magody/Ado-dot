...
public class HEsperanza : Habilidad {
	...
	void Start () {
		SuperStart ();
		nombre = "Esperanza";
		prefab_efecto = Resources.Load("PrefabsCargados/Habilidades/prefab_esperanza") as GameObject;
		...
		if (!esta_reutilizacion) {
			if (esta_activada) {
				...
				if (tiempo_segundo_contador > 1) {
					ManejadorCombate.sanacionFlotanteAlJugador (this.transform, jugadorEstadisticas.Vida_base [jugadorEstadisticas.Nivel_actual] * 0.01f, jugadorEstadisticas, 2f); //1%
