...
public class Destrezas : MonoBehaviour {
	...
	public void entrenar(){
		...
		if (esta_entrenando) {
			...
			if (tema_entrenamiento == "FMD") {
				...
				for (int i = 0; i < jugadorControlador.JugadorEstadisticas.Fuerza_magica_destructora_base.Length; i++) {
					jugadorControlador.JugadorEstadisticas.Fuerza_magica_destructora_base [i]++;
				}


			} else if (tema_entrenamiento == "FMS") {
			
				for (int i = 0; i < jugadorControlador.JugadorEstadisticas.Fuerza_magica_sanadora_base.Length; i++) {
					jugadorControlador.JugadorEstadisticas.Fuerza_magica_sanadora_base [i]++;
				}
			} else if (tema_entrenamiento == "FF") {

				for (int i = 0; i < jugadorControlador.JugadorEstadisticas.Fuerza_fisica_base.Length; i++) {
					jugadorControlador.JugadorEstadisticas.Fuerza_fisica_base [i]++;
				}
			}

			jugadorControlador.JugadorEstadisticas.actualizarEstadisticasNivel ();

			if (meta_entrenamiento_contador == meta_entrenamiento) {
				//acabó el entrenamiento
				esta_entrenando = false;
				DialogManager.IniciarDialogo (new string[]{ "Tema: " + tema_entrenamiento + " completado" });
			...
			case MAGIA_DESTRUCCION:
				...
				if (!destrezasMagiaDestruccion.estaVacia()) {
					...
					if (tema_entrenamiento == "FMD") {
						...
						DialogManager.IniciarDialogo (new string[]{"Tema: " + tema_entrenamiento + ", numero de entrenamientos: " + meta_entrenamiento});

					} else if (tema_entrenamiento == "Habilidad") {
						meta_entrenamiento = 5;
						DialogManager.IniciarDialogo (new string[]{"Se ha entrenado la habilidad: " + tokens [1] + " y la tiene que usar " + meta_entrenamiento + " veces."});

					}

					esta_entrenando = true;
				} else {
					DialogManager.IniciarDialogo (new string[]{"El entrenamiento ha sido completado ya."});
				}
				break;
